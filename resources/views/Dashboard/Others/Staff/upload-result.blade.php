...
@section('contents')
        ...
        <div class="container-fluid">
            <div class="col-lg-12">
                <div class="dash-item">
                    <h6 class="item-title"><i class="fa fa-info-circle"></i>@if(empty($a_performance))Add @else Update @endif Students Grade</h6>
                    @if(empty($a_performance))
                    ...
                    @else
                        <div class="inner-item">
                            <form method="post" action="{{route('user.process-update-student-result')}}">
                                @csrf
                                <table id="attendenceDetailedTable" class="display responsive nowrap" cellspacing="0" width="100%">
                                    <thead>
                                    <tr>
                                    ...
                                    @foreach($students as $key => $student)
                                        <tr>
                                            ...
                                            <td>{{$student->faculty->faculty_name}}</td>
                                            <td>{{$student->department->department_name}}</td>
                                            <td><input name="total_point_{{$key}}" type="number" value="{{$a_performance[$key]->total_point}}" placeholder="{{$a_performance[$key]->total_point}}" class="datatable-input" required/></td>
                                            <td><input name="total_unit_{{$key}}" type="number" value="{{$a_performance[$key]->total_unit}}" placeholder="{{$a_performance[$key]->total_unit}}" class="datatable-input" required/></td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                                ...
                                <input name="maximum_number" value="{{count($students)}}" hidden />
                                <div class="table-action-box">
                                    <button id="submit-button"><i class="fa fa-check"></i>Update Result</button>
                                </div>
                            </form>
                        </div>
                    @endif
